...
class SampleExample:
    ...
    clip_metadata: examplegen.ClipMetadata = examplegen.ClipMetadata(
        filename='Kauai_A_01_091214_194000.df20.x.flac',
        sample_rate=sample_rate,
        duration=datetime.timedelta(seconds=duration),
        index_in_file=18,
        start_relative_to_file=datetime.timedelta(seconds=24.96),
        start_utc=datetime.datetime(2009, 12, 14, 21, 2, 54),
    )
    ...
    annotations: Iterable[examplegen.ClipAnnotation] = dataclasses.field(
        default_factory=lambda: [
            examplegen.ClipAnnotation(
                begin=datetime.timedelta(seconds=24.96),
                end=datetime.timedelta(seconds=0.96),
                label='Mn')
        ])
...
class TestDataset(unittest.TestCase):
    ...
    def test_extract_window_labels(self):
        ...
        self.assertEqual(['Mn', 'Oo'], list(
            tf.boolean_mask(CLASS_NAMES, labels)))
    ...
    def test_sliding_windows(self):
        ...
        expected_window_count = _window_count(
            clip_duration, window_duration, hop)
        self.assertEqual(
        ...
        with _temp_tfrecords(examples) as infile:
            for batch_size in [2, 4]:
                ...
                window_duration_samples = int(
                    window_duration * Example().sample_rate)
    ...
    def test_new_window_dataset_sliding(self):
        ...
        def test_new_window_dataset_custom(self):
            window_duration = 1.0
            hop = 0.5
            class_names = ['Other', 'Mn']
            examples = _integration_test_fixture_examples()
            with _temp_tfrecords(examples) as infile:
